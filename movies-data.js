...
<head>
<meta charset="UTF-8">
<title>BUSANSO – Movie Player</title>
...
<div class="details">
  ...
  <button class="watch" id="watchBtn">▶ Watch Now</button>
  <p id="synopsis"></p>
...
if(!id) location.replace("movies-list.html");

const movie = MOVIES.find(m => m.id === id);
if(!movie) location.replace("movies-list.html");

// Populate info
title.textContent = movie.title;
poster.src = movie.poster;
meta.textContent = `${movie.type} • ${movie.duration} • ${movie.country} • ${movie.year}`;
...
function normalizeVideo(url){
  try {
    url = url.trim();
    if(url.includes("/embed/") || url.includes("player.vimeo.com") || url.includes("/preview")) return url;
...
// ===== PLAYBACK =====
function playMovie() {
  if (isFreeUser()) openAd(); // show ad for free users
  const url = normalizeVideo(movie.video);
...
// ===== COMMENTS LOGIC =====
const commentsRef = firebase.database().ref("comments/movie/"+movie.id);
...
commentBtn.onclick = () => {
  ...
  if(name.length < 2 || name.length > 20){
    ...
    commentMsg.textContent="Nickname must be 2–20 characters";
    return;
  }
  if(text.length < 2 || text.length > 300){
    commentMsg.style.color="#ffb400";
    commentMsg.textContent="Comment must be 2–300 characters";
    return;
  }
